...
나이브하게 `a * a`를 하면 Karatsuba로 Fp 곱셈 3회. 하지만 `a = b`를 활용하면:
- `c₀ = a₀² - a₁² = (a₀+a₁)(a₀-a₁)` → 곱셈 **1회**
- `c₁ = 2·a₀·a₁` → 곱셈 **1회** (사실 `a₀*a₁`을 두 번 더하는 것)

> [!tip] 제곱이 곱셈보다 싼 이유
> 일반 곱셈: Fp 곱셈 3회 (Karatsuba)
> 제곱: Fp 곱셈 **2회**
> Miller loop에서 매 비트마다 `f.square()`가 호출되므로, 이 차이가 누적된다.
